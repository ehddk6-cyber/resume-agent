...
def run_wizard(
    ...
    if global_experiences:
        if Confirm.ask(
            f"\n🌍 Global Vault에서 {len(global_experiences)}개의 경험을 불러오시겠습니까?",
            default=True,
        ):
    ...
    if import_path and import_path.exists():
        ...
        console.print(
            f"✅ [green]{len(imported)}개 경험을 추가로 가져왔습니다.[/green]"
        )
    elif import_path:
    ...
    if not experiences:
        if Confirm.ask(
            "\n📁 기존 로컬 경험 데이터 파일(.docx 등)을 가져오시겠습니까?",
            default=False,
        ):
            file_path = Prompt.ask(
                "📄 경험 파일 경로", default="취업/경험정리/경험요약정리.docx"
            )
            imported = import_experiences_from_file(Path(file_path))
            if imported:
                experiences.extend(imported)
                console.print(
                    f"✅ [green]{len(imported)}개 경험을 가져왔습니다.[/green]"
                )
    ...
    if experiences:
        ...
        if verified_count > 0:
            console.print(
                f"✅ [bold green]{verified_count}개의 경험이 L3/VERIFIED로 자동 승격되었습니다![/bold green]"
            )
    ...
    company_type = Prompt.ask("기업 유형", choices=company_type_choices, default="공공")

    research_notes = ""
    auto_questions = []
    if not jd_path:
        if Confirm.ask(
            "\n📄 직무기술서(PDF) 또는 여러 JD가 있는 폴더를 분석하여 핵심 키워드를 추출하시겠습니까?",
            default=False,
        ):
            jd_path_str = Prompt.ask(
                "PDF 파일 또는 폴더 경로", default="취업/직무기술서/"
            )
    ...
    if jd_path and jd_path.exists():
        ...
        # JD 구조 분석 및 질문 자동 생성
        from .pdf_utils import analyze_jd_structure, generate_questions_from_jd

        jd_analysis = analyze_jd_structure(jd_text)
        auto_questions = generate_questions_from_jd(jd_analysis)

        if auto_questions:
            console.print(
                f"\n✨ [bold green]JD에서 {len(auto_questions)}개 예상 문항이 생성되었습니다![/bold green]"
            )
    ...
    # 자동 생성 질문이 있으면 먼저 제안
    if jd_path and jd_path.exists() and auto_questions:
        if Confirm.ask("자동 생성된 문항을 사용하시겠습니까?", default=True):
            for aq in auto_questions:
                detected_type = classify_question(aq["question_text"])
                questions.append(
                    Question(
                        id=f"q{question_num}",
                        order_no=question_num,
                        question_text=aq["question_text"],
                        char_limit=aq.get("char_limit"),
                        detected_type=detected_type,
                    )
                )
                console.print(f"  ✅ Q{question_num}: {aq['question_text'][:50]}...")
                question_num += 1
...
def _parse_star_text(text: str) -> List[Experience]:
    ...
    for match in block_pattern.finditer(text):
        ...
        if any(
            c in result for c in ["%", "건", "명", "배", "단축", "절감", "증가", "감소"]
        ):
...
def _recommend_experiences(
    question: Question, experiences: List[Experience]
) -> List[Experience]:
    """문항에 맞는 경험 추천"""
