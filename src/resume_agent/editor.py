...
from .state import Workspace, load_experiences, save_experiences, initialize_state
...
def run_editor(ws: Workspace) -> None:
    ...
    if not experiences:
        console.print("[yellow]현재 등록된 경험이 없습니다. 먼저 wizard나 mine-past를 통해 경험을 추가하세요.[/yellow]")
        return
    ...
    while True:
        ...
        if choice.isdigit() and 1 <= int(choice) <= len(experiences):
            ...
            if new_metrics and new_metrics != "정량 수치 없음":
                ...
                console.print("[yellow]수치 데이터가 감지되어 증거수준이 L3로 조정되었습니다.[/yellow]")
