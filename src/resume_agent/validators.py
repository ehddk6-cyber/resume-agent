...
class ExperienceValidator:
    ...
    # STAR 구조 최소 길이 (config.yaml에서 로드)
    _star_lengths = get_config_value("validation.star_min_lengths", {})

    MIN_SITUATION_LENGTH = _star_lengths.get("situation", 50)
    MIN_TASK_LENGTH = _star_lengths.get("task", 30)
    MIN_ACTION_LENGTH = _star_lengths.get("action", 100)
    MIN_RESULT_LENGTH = _star_lengths.get("result", 50)
    ...
    def validate(self, experience: Experience) -> ExperienceValidationResult:
        """
        경험 데이터 종합 검증
...
def validate_experience(experience: Experience) -> ExperienceValidationResult:
    """경험 검증 편의 함수"""
