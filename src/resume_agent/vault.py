...
class VaultManager:
    ...
    def save_global_experiences(self, experiences: List[Experience]) -> None:
        ...
        self.global_experiences_file.write_text(json.dumps(data, ensure_ascii=False, indent=2), encoding="utf-8")
    ...
    def sync_to_global(self, local_experiences: List[Experience]) -> None:
        ...
        for local_exp in local_experiences:
            if local_exp.id in global_exps:
                ...
                # 내용이 다른 경우에만 날짜 비교
                if local_exp.model_dump(exclude={"updated_at"}) != global_exp.model_dump(exclude={"updated_at"}):
                    # 로컬이 더 최신인 경우에만 갱신
        ...
        if updated_count > 0 or new_count > 0:
            ...
            print(f"🌍 Global Vault 동기화: {new_count}개 신규, {updated_count}개 갱신 완료 (백업 생성됨)")
